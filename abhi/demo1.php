...
<li>
  ...
  <?php
    ...
    $to=substr($dest,stripos($dest,"-")+1);
     ...
     $s=mysqli_fetch_assoc(mysqli_query($con,"select stopNum from route where stationCode='$from' and trainNum='$train'"))['stopNum']; 
   
     $d=mysqli_fetch_assoc(mysqli_query($con,"select stopNum from route where stationCode='$to' and trainNum='$train'"))['stopNum'];  

     $dt=mysqli_query($con,"select date from traindate where date>='$date' and trainNum='$train' order by date"); 

     $f=mysqli_fetch_assoc(mysqli_query($con,"select farePerKm from train,fareinfo where trainNum='$train' and train.type=fareinfo.trainType and classCode='$class'"))['farePerKm'];
     $sd=mysqli_fetch_assoc(mysqli_query($con,"select distance from route where stopNum='$s' and trainNum='$train'"))['distance'];
     $dd=mysqli_fetch_assoc(mysqli_query($con,"select distance from route where stopNum='$d' and trainNum='$train'"))['distance'];
     $fare=floor($f*($dd-$sd));
     $rc=40;
    $tc=0;
     if($class=='SL')
	 ...
	 while($bookingDate=mysqli_fetch_assoc($dt)) 
	{
       ...
       $id=mysqli_fetch_assoc(mysqli_query($con,"select train_ID from traindate 
                                          where trainNum='$train' and date='$date' "))['train_ID'];

   $quota='GN';
   
   $st=mysqli_query($con,"select NumOfSeats,stopNum from seatsAvailInfo where stopNum>='$s' 
      ...
      {
        // for RAC seats
         $st=mysqli_query($con,"select NumOfSeats,stopNum from seatsAvailInfo 
		                       where stopNum>='$s' and stopNum<='$d' and train_ID='$id' and 
                               classCode='$class' and quotaCode='RAC' ");
			
			$total=mysqli_fetch_assoc(mysqli_query($con,"select sum(numOfSeats) numOfSeats from seatsAvailInfo 
			                                         where stopNum>='$s' and stopNum<='$d' and train_ID='$id' and 
      ...
      if($min==0&&$status=='RAC')
        {
          $st=mysqli_query($con,"select waiting,stopNum from trainwaitingseatinfo 
