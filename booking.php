...
include('header.php');
  ...
  $s=mysqli_fetch_assoc(mysqli_query($conn,"select stopNum from route where stationCode='$src' and trainNum='$trainNum'"))['stopNum']; 
  $d=mysqli_fetch_assoc(mysqli_query($conn,"select stopNum from route where stationCode='$dest' and trainNum='$trainNum'"))['stopNum'];  

  $station=mysqli_query($conn,"select stationCode from route where stopNum>='$s' and stopNum<'$d' and trainNum='$trainNum' ");
  $srcStationName=mysqli_fetch_assoc(mysqli_query($conn,"select stationName from station 
  	                                                    where stationCode='$src'"))['stationName'];
    $destStationName=mysqli_fetch_assoc(mysqli_query($conn,"select stationName from station 
...
<li>
	<a href="logout.php" class="btn btn-lg" style="color: black" name="logout">
	  ...
	  <div class="container">	
	  			...
	  			<td>
	  				<select name="boarding">	
	  					...
	  					while($stCode=mysqli_fetch_assoc($station))
	  					{
	  						...
	  						$stName=mysqli_fetch_assoc(mysqli_query($conn,"select stationName from station where 
         ...
         <a href="userhome.php" class="btn btn-primary" style="margin-left:20px;margin-top: 10px">Replan</a>
    </form>
    	</div>
	  </div>
